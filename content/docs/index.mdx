...
|---|---|---|
| `anvay-core` | FastAPI backend, retrieval, council, MCP server | `git clone https://github.com/0xaeres/anvay-core.git anvay` |
...
Most users need only `anvay` and `anvay-ui`. The [Quickstart](/docs/quickstart) walks through both from an empty directory; [Deployment](/docs/deployment) covers running them in production.
