...
The documented `docker-compose.prod.yml` does not include FalkorDB. Graph-dependent behavior — `ask_product_graph` and the graph-local pass inside `evidence_search_corpus` — needs a FalkorDB instance reachable from the API container. If you rely on those, add a FalkorDB service to your Compose file (mirroring the dev `docker-compose.yml`) and point `anvay.yaml` at it before going to production.
...
| `ANVAY_ENABLE_LOCAL_FS_SOURCES` | whether local-filesystem sources can be configured at all; keep `false` in production |
...
Open the UI root and sign in with the bootstrap administrator credentials. On backend startup, the bootstrap email is ensured as an approved `admin` account and its password is reset from the bootstrap environment variables — this happens on every restart, so rotate or remove those variables once you no longer want that behavior.
...
Other users sign up at `/request-access` and remain pending until an administrator approves them from `/admin/access`. Approval assigns an app role (`admin`, `editor`, or `viewer`); revoking a user blocks future backend access immediately. There is no self-service admin promotion — every non-bootstrap admin is granted by an existing admin.
...
- Non-admin users see only their own products; a fresh signup at `/request-access` stays pending until approved at `/admin/access`.
- Product viewers cannot manage sources or run council.
- Local filesystem sources are rejected when `ANVAY_ENABLE_LOCAL_FS_SOURCES=false`.
