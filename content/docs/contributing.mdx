---
title: "Contributing"
description: "Set up the repositories, find the right module, preserve Anvay's invariants, and ship a focused change."
slug: "contributing"
group: "Develop"
order: 60
sourceLabel: ""
editUrl: ""
---
Anvay has a Python backend and a Next.js frontend. Most changes cross a clear API boundary: the backend defines product-scoped behavior and typed contracts; the UI exposes that behavior without inventing parallel business logic.

## Development architecture

```mermaid
flowchart LR
  UI["Next.js UI"] --> Client["Typed API client"]
  Client --> Routes["FastAPI routes"]
  Routes --> Registry["Registry + auth"]
  Routes --> Connectors["Connectors + daemon"]
  Connectors --> Ingest["Ingestion"]
  Routes --> Retrieval["Retrieval"]
  Routes --> Council["Council"]
  Routes --> Skills["Approval + skills"]
  Council --> Retrieval
  Ingest --> Qdrant["Qdrant"]
  Ingest --> Graph["FalkorDB graph"]
  Retrieval --> Qdrant
  Retrieval --> Graph
  Skills --> Git["Skills Git repository"]
  MCP["MCP server"] --> Retrieval
  MCP --> Skills
  CLI["anvay CLI"] --> Ingest
  CLI --> Retrieval
  CLI --> Council
```

## Before you edit

Read the repository instructions first:

1. `anvay/AGENTS.md` for backend invariants and validation commands.
2. `anvay-ui/AGENTS.md` and `anvay-ui/DESIGN.md` before changing screens, routes, or visual components.
3. `anvay/ENGINEERING.md` when changing data models, retrieval, council behavior, or API contracts.

Two rules dominate code review:

- **Product is the root entity.** Every resource and query remains product-scoped.
- **Humans approve, agents draft.** No generated proposal becomes an approved skill without explicit user action.

## Repository map

| Area | Location | Use it for |
|---|---|---|
| CLI | `anvay/cli.py` | Typer entry point: `ingest`, `query`, `council draft`, `eval run`, `daemon`, `delete-product` |
| API routes | `anvay/api/routes/` | HTTP contracts, authorization, product workflows |
| Connectors | `anvay/connectors/` | GitHub, Jira, Confluence, local filesystem, remote MCP, plus the source manager |
| Daemon | `anvay/daemon.py` | continuous re-sync loop for `watch: true` connectors |
| Ingestion | `anvay/ingest/` | chunking, manifests, embeddings, derived-index updates |
| Retrieval | `anvay/retrieval/` | dense and lexical retrieval, fusion, reranking, evidence assembly |
| Graph | `anvay/graph/` | tree-sitter extraction, bounded LLM fact layer, FalkorDB store, GraphRAG traversal |
| Council | `anvay/council/` | LangGraph execution (planner, synthesizer, repair, eval, finalizer), proposal queue |
| Skills | `anvay/skills/` | models, approval, storage, Git publication |
| MCP server | `anvay/mcp_server/` | tools exposed to coding clients |
| UI screens | `anvay-ui/components/screens/` | one screen component per route |
| UI API client | `anvay-ui/lib/api/index.ts` | typed endpoint access |
| UI domain types | `anvay-ui/lib/types.ts` | shared frontend contracts |

## Local development

Start backend dependencies and the API:

```bash
cd anvay
uv sync
cp anvay.yaml.example anvay.yaml
cp .env.example .env
make services-up
uv run uvicorn anvay.api.app:app --port 8000 --reload
```

Start the UI in a second terminal:

```bash
cd anvay-ui
npm install
npm run dev
```

Open `http://localhost:3000`. Public landing and docs routes can render independently; authenticated product routes require the backend.

## Follow an end-to-end change

When adding or changing a user-visible capability, trace it in this order:

1. Define or update the backend model.
2. Add the product-scoped route and authorization check.
3. Add focused backend tests.
4. Add one typed function in `anvay-ui/lib/api/index.ts`.
5. Reuse types from `anvay-ui/lib/types.ts`.
6. Update the existing screen or add one screen component under `components/screens/`.
7. Verify empty, loading, success, permission, and error states.

Do not add a UI-only field that the backend cannot serve. Do not call backend routes directly from components when the API client already owns that boundary.

```mermaid
flowchart TD
  Need["User-visible requirement"] --> Contract["Backend model + contract"]
  Contract --> Auth["Product authorization"]
  Auth --> Tests["Backend tests"]
  Tests --> Client["Typed UI accessor"]
  Client --> Screen["Screen behavior"]
  Screen --> States["Loading, empty, error, permission states"]
  States --> Build["Production build"]
  Build --> Visual["Browser verification"]
```

## Test your change

Backend:

```bash
uv run ruff check anvay tests evals
uv run pytest -q
```

Frontend:

```bash
npm run build
npm run dev
```

Run the eval harness after changing chunking, embeddings, sparse encoding, fusion, reranking, repo maps, evidence assembly, or council prompts. It has three suites, each with a hard-gated threshold:

```bash
uv run anvay eval run --suite retrieval   # Recall@10 ≥ 0.80, MRR ≥ 0.50
uv run python -m evals.run_ragas          # Faithfulness ≥ 0.85, Answer correctness ≥ 0.80, Context recall ≥ 0.75
uv run python -m evals.run_code_eval      # nDCG@10 ≥ 0.75, Recall@10 ≥ 0.80, Pairwise preference ≥ 0.85
```

A pull request that touches retrieval or council behavior should report before/after numbers from these suites, not just "looks better." See `evals/README.md` for harness details.

### Match validation to risk

| Change | Minimum validation |
|---|---|
| Text or narrow visual adjustment | frontend build and affected viewport |
| API response or permission | route tests, frontend build, denied and allowed cases |
| Ingestion behavior | unit tests, sync integration test, delta behavior |
| Retrieval behavior | unit tests, retrieval eval suite, latency comparison |
| Graph extraction or traversal | unit tests, code eval suite (nDCG, recall), spot-checked traversal |
| Council behavior | graph tests, completeness checks, proposal-gate verification, RAGAS suite |
| Approval path | tests proving Git write precedes approved status |

## Common contribution paths

### Add an API endpoint

Keep request and response data in Pydantic models, apply product authorization, and add a test for the public route. The frontend accessor should remain thin: one function per endpoint, no business decisions.

### Add a connector

A connector is incomplete until it can enumerate stable resources, read content, participate in manifest diffing, report progress, and preserve product scope. Look at `anvay/connectors/jira.py` or `anvay/connectors/confluence.py` for the current shape before adding a new one; `anvay/connectors/manager.py` is where a connector gets wired into sync and the watch daemon. UI exposure comes after backend synchronization works.

### Change council behavior

Preserve the bounded graph and proposal gate: planner assembles context once, the synthesizer writes the full skill in one call, repair is capped at three attempts, and incomplete output cannot reach review. Add deterministic checks in `anvay/council/skill_evals.py` where possible before reaching for another LLM judge, and keep evidence attached to material claims. Test with `uv run anvay council draft --product <id> --topic "..."` before wiring a change through the UI.

### Tune retrieval or the graph

Treat eval results as part of the implementation. Record the baseline, change one meaningful variable, and compare recall, ranking quality, faithfulness, and latency before adopting the change. `uv run anvay query "..." --product <id>` runs the retrieval pipeline directly, without the API or a council session, which is the fastest loop for iterating on chunking, fusion, or reranking changes. See the [CLI reference](/docs/cli) for the full command set.

### Change documentation

Public docs live in the separate `anvay-docs` repository and are fetched from Tina Cloud. Write task-oriented MDX there, validate the curated files, commit and push `main`, then wait for Tina indexing before verifying `/docs`.

Use fenced `mermaid` blocks for architecture or lifecycle diagrams. Keep node labels short, avoid diagrams wider than the article, and pair every diagram with prose that explains why the flow matters.

## Debugging approach

Follow the first incorrect boundary rather than patching the final symptom:

1. Reproduce with one product and one request.
2. Inspect the browser network or API response.
3. Confirm the typed frontend contract.
4. Read the route and authorization path.
5. Trace the domain service and durable-state write.
6. Inspect derived indexes only after source-of-truth state is understood.

For asynchronous workflows, use session events and source progress logs to locate the stage that failed.

## Pull request checklist

- The change stays inside one product boundary.
- Generated content still requires human approval.
- Public routes and types have tests.
- Backend lint and tests pass.
- Frontend production build passes.
- Screens follow the locked information architecture and design tokens.
- No unrelated refactor or generated artifact is included.
