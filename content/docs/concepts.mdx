...
SQLite manifests record what completed successfully. Qdrant (vectors), the FalkorDB product graph, and repository maps are derived serving state. If a derived index is lost, Anvay can rebuild it from the configured sources and manifests; it must not infer sync truth from whatever happens to remain in the vector database or graph.
...
Dense and BM25 are the core of the fan-out, but retrieval draws on more channels: literal `grep`, the repository map, the FalkorDB product graph for structural relationships, and approved skills. The `evidence_search_corpus` and `ask_product_graph` tools combine these so an answer can cite both matching source spans and the graph relationships around them.
